...
const RideOptionsCard = () => {
    ...
    const onChoose = () =>{
        Alert.alert('Felicidades!!!', `Ruta: ${selected.title} \nDistence: ${travelTimeInformation?.distance?.text} \n${travelTimeInformation?.duration.text} Tiempo de viaje \nSale en 15 minutos`)
    }
        ...
        <Screen style={tailwind`bg-white h-full`}>
            ...
            <View>
                <TouchableOpacity
                    ...
                    onPress={onChoose}
                >
                    <Text style={tailwind`text-center text-white text-xl`}>Elige {selected?.title}</Text>
                </TouchableOpacity>
            </View>
        </Screen>
    )
